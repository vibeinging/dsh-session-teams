...
var ConversationWindowDirectory = class {
	...
	async resolve(source, selectors, signal) {
		if (selectors.targetName !== void 0 && selectors.targetLink !== void 0) return failure("", "multiple-targets", "provide target_name or target_link, not both");
		...
		if (selectors.targetLink !== void 0) {
			...
			if (matches.length === 0) return failure("", "target-name-not-found", "no conversation has that exact title; call list_conversation_windows to see the exact displayed titles");
			if (matches.length > 1) return failure("", "target-name-ambiguous", "more than one conversation has that title");
			target = matches[0];
		} else {
			if (windows.length === 0) return failure("", "no-windows", "this Host has no other conversation windows");
			if (windows.length > 1) return failure("", "target-required", "target_name is required when this Host has several conversations");
			target = windows[0];
		}
	...
	renderModelContext(source, maxEntries) {
		...
		const rows = shown.map((window) => {
			return `- title ${window.title === void 0 ? "(untitled)" : JSON.stringify(window.title)}; ${window.running ? "working" : "available"}; link ${serializeWindowLink(window.sessionId)}`;
		});
		return [
...
function renderListResult(value) {
	return [
		...
		`code: ${value.code}`,
		`message: ${value.message}`,
		...value.windows.map((window) => `- title: ${window.title === null ? "(untitled)" : JSON.stringify(window.title)}; state: ${window.state}; target_link: ${window.targetLink}`)
	].join("\n");
}
...
function createWindowTeamTool(config, provisioner, directory, coordinator, scheduler, workspaceRegistry) {
	return defineTool({
		...
		async execute(args, exec) {
			...
			for (const member of normalizedMembers) if (existing.filter((window) => window.title === member.name).length > 1) return rejectedTeam(teamId, "target-name-ambiguous", `more than one conversation is named ${JSON.stringify(member.name)}`);
...
function createAddWindowTaskTool(config, directory, coordinator, scheduler) {
	return defineTool({
		...
		description: "Add one new task after a window team already exists. Do not use this to build a graph known during create_window_team; put that graph in members[].tasks instead. Name an exact owner window and optional dependencies. Ready work starts automatically. Use only for a direct user request.",
		parameters: {
			target_name: {
				type: "string",
				required: true,
				description: "Exact displayed title of the owner window."
			},
			title: {
				type: "string",
				required: true,
...
function createReassignWindowTaskTool(config, directory, coordinator, scheduler) {
	return defineTool({
		...
		description: "Manually move a blocked, ready, or failed team task to another visible conversation. Running and completed work is not silently duplicated. Use only for a direct user request.",
		parameters: {
			task_id: {
				type: "string",
				required: true,
			...
			target_name: {
				type: "string",
				required: true,
				description: "Exact displayed title of the new owner window."
			},
			owner_role: {
				type: "string",
