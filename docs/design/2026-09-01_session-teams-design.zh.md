...
`target_link` 是首要寻址方式：目录里列出的每个窗口都带有规范的 `dsh://session/...` 链接，模型被引导向窗口的链接发送，而不是仅凭标题，因为标题可能重复或被改名。`target_name` 仍是可选兜底，执行标准化后的准确标题匹配，只有当恰好一个对话拥有该标题时才有效；标题重复时会被拒绝，并提示使用目标窗口的 `target_link`。两个选择器同时提供时，链接优先。两个选择器都省略时，只有工作区中正好存在一个其他对话才会自动选择。
...
可选 `SystemPrompt` 内容会列出一个有数量限制的目录快照，包含链接、标题和活动状态。它会说明每个条目都可以通过其 `target_link` 直接访问，标题只是显示名（单独并不能作为安全地址），而且自然语言中的停止偏好应通过上下文遵守，不能修改目录。对于用户直接提出的团队请求，它会要求模型直接调用 `create_window_team`，在一次调用中把所有已知的有序工作放入 `members[].tasks`，不要计划日后再连续调用 `add_window_task`。它还会说明标题完全一致且唯一的对话会被复用，标题有歧义时必须先澄清。缓存信息不够时，`list_conversation_windows` 会异步刷新完整列表。
